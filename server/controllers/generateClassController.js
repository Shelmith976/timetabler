...
const generateMain = (data) => {
	...
	for (let i = 0; i < options.size; i++) {
		const solution = seed();
		const sln = localSearch(solution, data);
		population.push(sln);
	}
...
const localSearch = function (solution, data) {
	for (let i = 0; i < solution.length; i++) {
		for (let j = i + 1; j < solution.length; j++) {
			...
			if (sameDay && sameTime) {
				if (sameRoom) {
					// Check for available rooms and assign the next available one
					for (let roomNum in data.roomCapacity) {
						const isRoomFree = solution.every((classObj) => {
							return (
								classObj.roomNum !== roomNum ||
								classObj.dayOfWeek !== solution[i].dayOfWeek ||
								classObj.endTime <= solution[i].startTime ||
								classObj.startTime >= solution[i].endTime
							);
						});
						const isCapacitySufficient =
							data.roomCapacity[roomNum] >=
							data.batchSizeMap[solution[i].batchCode];
						if (isRoomFree && isCapacitySufficient) {
							solution[i].roomNum = roomNum;
							break;
						}
					}
				}

				if (sameDay && sameTime && (sameLecturer || sameRoom || sameBatch)) {
					let foundSlot = false;
					// Try to find a new time slot on the same day
					for (let hour = 8; hour <= 16; hour++) {
						const newStartTime = `${hour}:00:00`;
						const classDuration = data.subjectLab[
							data.classSubject[solution[i].classId]
						]
							? 3
							: 2;
						const newEndTime = `${hour + classDuration}:00:00`;

						const isTimeSlotFree = solution.every((classObj) => {
							return (
								(classObj.staffno !== solution[i].staffno ||
									classObj.dayOfWeek !== solution[i].dayOfWeek ||
									classObj.endTime <= newStartTime ||
									classObj.startTime >= newEndTime) &&
								(classObj.roomNum !== solution[i].roomNum ||
									classObj.dayOfWeek !== solution[i].dayOfWeek ||
									classObj.endTime <= newStartTime ||
									classObj.startTime >= newEndTime) &&
								(classObj.batchCode !== solution[i].batchCode ||
									classObj.dayOfWeek !== solution[i].dayOfWeek ||
									classObj.endTime <= newStartTime ||
									classObj.startTime >= newEndTime)
							);
						...
						if (isTimeSlotFree) {
							solution[i].startTime = newStartTime;
							solution[i].endTime = newEndTime;
							foundSlot = true;
							break;
						}
					}

					// If no slot found on the same day, try to find a slot on a different day
					if (!foundSlot) {
						for (let day = 1; day <= 5; day++) {
							// Assuming week days are 1 to 5
							for (let hour = 8; hour <= 16; hour++) {
								const newStartTime = `${hour}:00:00`;
								const classDuration = data.subjectLab[
									data.classSubject[solution[i].classId]
